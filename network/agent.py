...
class Agent(object):
    ...
    def __init__(
        ...
        # set up networks
        self.__setup_networks(obs_dim, act_dim, hidden_sizes, act_limit,
                              device, num_Q)
        # set up optimizers
        self.__setup_optimizers(lr, num_Q)
        # set up adaptive entropy (SAC adaptive)
        self.__setup_adaptive_entropy(auto_alpha, target_entropy, lr, env_name,
                                      device)
        # set up replay buffer
        self.__setup_replay_buffer(obs_dim, act_dim, replay_size)
        # set up other things
        self.__setup_loss()

        # store other hyperparameters
        self.__setup_hyperparameters(start_steps, obs_dim, act_dim, act_limit,
                                     lr, hidden_sizes, gamma, polyak,
                                     replay_size, alpha, batch_size, num_min,
                                     num_Q, utd_ratio, delay_update_steps,
                                     q_target_mode, policy_update_delay,
                                     device)
    ...
    def get_action(self, obs, env=None, test=True):
        if (test):
            # given an observation, output a deterministic action in numpy form
            with torch.no_grad():
                obs_tensor = torch.Tensor(obs).unsqueeze(0).to(self.device)
                action_tensor = self.policy_net.forward(
                    obs_tensor, deterministic=True, return_log_prob=False)[0]
                action = action_tensor.cpu().numpy().reshape(-1)
            return action
        else:
            # given an observation, output a deterministic action in numpy form
            with torch.no_grad():
                if self.__get_buffer_size() > self.start_steps:
                    obs_tensor = torch.Tensor(obs).unsqueeze(0).to(self.device)
                    action_tensor = self.policy_net.forward(
    ...
    def get_ave_q_prediction_for_bias_evaluation(self, obs_tensor,
                                                 acts_tensor):
        # given obs_tensor and act_tensor, output Q prediction
        q_prediction_list = []
        for q_i in range(self.num_Q):
            q_prediction = self.q_net_list[q_i](torch.cat(
                [obs_tensor, acts_tensor], 1))
    ...
    def get_redq_q_target_no_grad(self, obs_next_tensor, rews_tensor,
                                  done_tensor):
        ...
        sample_idxs = np.random.choice(self.num_Q,
                                       num_mins_to_use,
                                       replace=False)
        with torch.no_grad():
            if self.q_target_mode == 'min':
                """Q target is min of a subset of Q values"""
                y_q = self.__q_target_min_mode(obs_next_tensor, rews_tensor,
                                               done_tensor, sample_idxs)
            if self.q_target_mode == 'ave':
                """Q target is average of all Q values"""
                y_q = self.__q_target_ave_mode(obs_next_tensor, rews_tensor,
                                               done_tensor, sample_idxs)
            if self.q_target_mode == 'rem':
                """Q target is random ensemble mixture of Q values"""
                y_q = self.__q_target_rem_mode(obs_next_tensor, rews_tensor,
                                               done_tensor, sample_idxs)
    ...
    def train(self, logger):
        ...
        num_update = 0 if self.__get_buffer_size(
        ...
        for i_update in range(num_update):
            ...
            if ((i_update + 1) % self.policy_update_delay
                    == 0) or i_update == num_update - 1:
                # get policy loss
                ...
                for sample_idx in range(self.num_Q):
                    ...
                    q_a_tilda = self.q_net_list[sample_idx](torch.cat(
                        [obs_tensor, a_tilda], 1))
            ...
            for q_i in range(self.num_Q):
                soft_update_model1_with_model2(self.q_target_net_list[q_i],
                                               self.q_net_list[q_i],
                                               self.polyak)
    ...
    def __setup_networks(self, obs_dim, act_dim, hidden_sizes, act_limit,
                         device, num_Q):
        self.policy_net = Policy(obs_dim,
                                 act_dim,
                                 hidden_sizes,
        ...
        for q_i in range(num_Q):
            ...
            new_q_target_net = Mlp(obs_dim + act_dim, 1,
                                   hidden_sizes).to(device)
            new_q_target_net.load_state_dict(new_q_net.state_dict())
            self.q_target_net_list.append(new_q_target_net)

    def __setup_optimizers(self, lr, num_Q):
        self.policy_optimizer = optim.Adam(self.policy_net.parameters(), lr=lr)
        self.q_optimizer_list = []
        for q_i in range(num_Q):
            self.q_optimizer_list.append(
                optim.Adam(self.q_net_list[q_i].parameters(), lr=lr))

    def __setup_adaptive_entropy(self, auto_alpha, target_entropy, lr,
                                 env_name, device):
    ...
    def __setup_replay_buffer(self, obs_dim, act_dim, replay_size):
    ...
    def __setup_hyperparameters(self, start_steps, obs_dim, act_dim, act_limit,
                                lr, hidden_sizes, gamma, polyak, replay_size,
                                alpha, batch_size, num_min, num_Q, utd_ratio,
                                delay_update_steps, q_target_mode,
                                policy_update_delay, device):
    ...
    def __q_target_min_mode(self, obs_next_tensor, rews_tensor, done_tensor,
                            sample_idxs):
        a_tilda_next, _, _, log_prob_a_tilda_next, _, _ = self.policy_net.forward(
            obs_next_tensor)
        q_prediction_next_list = []
        for sample_idx in sample_idxs:
            q_prediction_next = self.q_target_net_list[sample_idx](torch.cat(
                [obs_next_tensor, a_tilda_next], 1))
            q_prediction_next_list.append(q_prediction_next)
        q_prediction_next_cat = torch.cat(q_prediction_next_list, 1)
        min_q, min_indices = torch.min(q_prediction_next_cat,
                                       dim=1,
                                       keepdim=True)
        next_q_with_log_prob = min_q - self.alpha * log_prob_a_tilda_next
        y_q = rews_tensor + self.gamma * (1 -
                                          done_tensor) * next_q_with_log_prob
        return y_q

    def __q_target_ave_mode(self, obs_next_tensor, rews_tensor, done_tensor,
                            sample_idxs):
        a_tilda_next, _, _, log_prob_a_tilda_next, _, _ = self.policy_net.forward(
            obs_next_tensor)
        q_prediction_next_list = []
        for q_i in range(self.num_Q):
            q_prediction_next = self.q_target_net_list[q_i](torch.cat(
                [obs_next_tensor, a_tilda_next], 1))
            q_prediction_next_list.append(q_prediction_next)
        q_prediction_next_ave = torch.cat(q_prediction_next_list,
                                          1).mean(dim=1).reshape(-1, 1)
        next_q_with_log_prob = q_prediction_next_ave - self.alpha * log_prob_a_tilda_next
        y_q = rews_tensor + self.gamma * (1 -
                                          done_tensor) * next_q_with_log_prob
        return y_q

    def __q_target_rem_mode(self, obs_next_tensor, rews_tensor, done_tensor,
                            sample_idxs):
        a_tilda_next, _, _, log_prob_a_tilda_next, _, _ = self.policy_net.forward(
            obs_next_tensor)
        q_prediction_next_list = []
        ...
        q_prediction_next_rem = (q_prediction_next_cat *
                                 rem_weight).sum(dim=1).reshape(-1, 1)
